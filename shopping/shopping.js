...
(function() {
    ...
    class Product {
        ...
        getInfo() {
            return this.name + ' -> ' + this.name[0] + this.name[this.name.length - 1] + this.productId + ', ' + this.name + ', ' + this.price;;
        }
    }
    class ShoppingBag {
        constructor() {
        ...
        averagePrice() {
            ...
            let avg = (sum / this.products.length).toFixed(3);
            console.log('The average price of products in the bag is' + avg);
        }
        getMostExpensive() {
            let mostExpensive = this.products[0];
            for (let i = 1; i < this.products.length; i++) {
                if (parseFloat(this.products[i].price) > parseFloat(mostExpensive.price)) {
                    mostExpensive = this.products[i];
                }
            }
            console.log('\t'+'The most expensive product is:' + mostExpensive.getInfo());
        }
        calculateTotalPrice() {
            let total = 0;
            for (let i = 0; i < this.products.length; i++) {
            ...
            console.log('\t'+'The total price of products in the bag is $' + total.toFixed(2));
            return total;
        }
    }

    class PaymentCard {
        constructor(balance, status, validUntil) {
    ...
    try {
        let apple = new Product("Apple", 0.99, "2023-12-31");
        let banana = new Product("Banana", 0.59, "2024-01-01");
        let orange = new Product("Orange", 0.89, "2021-12-31");
        let bag = new ShoppingBag();

        bag.addProduct(apple);
        bag.addProduct(banana);
        bag.addProduct(orange); // Nece biti dodato jer je istekao rok upotrebe

        console.log(bag);

        var paymentCardVisa = new PaymentCard(500, "active", "2023.05.10");
        console.log('\t'+'Your payment card balance is: $' + paymentCardVisa.balance);

    checkoutAndBuy(bag.calculateTotalPrice(), paymentCardVisa.balance);
